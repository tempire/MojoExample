...
ok my $set_id =
  $t->tx->res->dom('div#photosets > div.photo')->[0]->attr('id'),
  'set id';
ok my $set_url =
  $t->tx->res->dom('div#photosets > div.photo > a')->[0]->attr('href'),
  'set url';
ok my $set_title =
...
ok my $photo_url =
  $t->tx->res->dom('div.photoset a.slide')->[0]->attr('href'), 'photo url';
